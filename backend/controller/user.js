        ...
        await bcrypt.hash(password,10).then(async(hashedPassword) => {
            ...
            try {
                await sendMails({email: user.email, subject: 'Activate your account', text: `Hello ${
                        user.name
                    } click on the following link to activate your account: ${activationUrl}`})
                    console.log('activation link was sent successfully')
                    console.log(activationUrl)
               return res.status(201).json({message: `please check your ${
                        user.email
...
).catch(err => {const token=user.getJwtToken()
   console.log(err)
})
}
const activateToken = (user) => {
    const token = jwt.sign(user, process.env.JWT_TOKEN_SECRET, {expiresIn: '5m'})
 ...
 const login=catchAsyncError(async(req,res,next)=>{
   ...
   if(!email||!password){
    ...
    return next(new errorHandler("please provide the required credentials",400))
   }
   else{
